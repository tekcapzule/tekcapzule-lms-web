export { CoreModule } from './core.module';
export { FeedApiService } from './services/feed-api/feed-api.service';
export { CourseApiService } from './services/course-api/course-api.service';
export { DashboradApiService } from './services/dashboard-api/dashboard-api.service';
export { AppSpinnerService } from './services/app-spinner/app-spinner.service';
export { ReferenceApiService} from './services/reference-api/reference-api.service';
